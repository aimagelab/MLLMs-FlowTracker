...
def pil_collate_fn(batch):
    image, question, answer_tokens, questionId = zip(*batch)  # Assuming each item is a tuple (PIL.Image, label)
    return list(image), list(question), list(answer_tokens), list(questionId)
...
class DocVQADataset(Dataset):
    def __init__(self, image_dir) -> None:
        ...
        with open("tracing_information_flow/dataset/docvqa/docvqa_dataset_correct_answers.json", "r") as fp:
    ...
    def __getitem__(self, index):
        ...
        return image, question, answer_tokens, questionId


def parse_args():
    parser = argparse.ArgumentParser(description="DOCVQA Eval")
    parser.add_argument(
        "--answer_path", type=str, default="results/results_cmflowinfo_docvqa"
    )
    parser.add_argument(
        "--model_path", type=str, 
    )
...
if __name__ == "__main__":
    ...
    dataset = DocVQADataset(image_dir=args.image_dir) 
