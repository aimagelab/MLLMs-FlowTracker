    ...
    def forward_with_attention_blocking(
        ...
        if block_type in ['last_to_last', 'image_to_last', 'question_to_last', 'image_to_question']:
            ...
            for layer_idx in range(n_layers):
                ...
                elif block_type == "question_to_last" and prompt_lengths is not None:
                    expanded_causal_mask[left_border:right_border, :, :, -1, :prompt_lengths - 1] = torch.finfo(inputs_embeds.dtype).min  #last token not included
                    #for i in range(_bsz): 
                    #    len_p = prompt_lengths[i].item()
                        #expanded_causal_mask[left_border:right_border, i, :, -1, :len_p - 1] = float("-inf")  #last token not included
                ...
                # Block question from attending to the image
                    ...
                    expanded_cross_attention_mask[left_border:right_border, :, :, :prompt_lengths - 1, :] = torch.finfo(inputs_embeds.dtype).min
                    #for i in range(_bsz): 
                    #    len_p = prompt_lengths[i].item()
                    #    expanded_cross_attention_mask[left_border:right_border, i, :len_p - 1, :, :] = -0.0
...
class MllamaForConditionalGeneration(MllamaPreTrainedModel, GenerationMixin):
    ...
    def forward_with_attention_blocking(
        ...
        output_layer_logits = self.language_model.forward_with_attention_blocking(
            ...
            block_type=block_type,
            k=k,
            prompt_lengths=prompt_lengths,                                  #seq_length
